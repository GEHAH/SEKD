...
class Args:
	def __init__(self) -> None:
		...
		self.model = 'KD_EGNN'
args = Args()
class SEKDLoadModel1(QWidget):
    close_signal = pyqtSignal(str)
    def __init__(self):
        super(SEKDLoadModel1, self).__init__()
    ...
    def initUI(self):
        self.setWindowTitle('SEKD_PPIS LoadModel')
        self.resize(800, 600)
    ...
    def run_model(self):
        ...
        if len(self.model_list) > 0 and not self.dataframe is None:
            try:
                ...
                metrics_ind = np.array(["{:.2%}".format(result_test['ACC']),"{:.2%}".format(result_test['PRE']),
                                        "{:.2%}".format(result_test['REC']),"{:.2%}".format(result_test['F1']),
                                        "{:.2%}".format(result_test['AUC']),"{:.2%}".format(result_test['AUPRC']),"{:.2%}".format(result_test['MCC']),
